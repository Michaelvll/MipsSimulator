...
void PipelineClass::Data_Preparation(int &state, int busyreg[4])
{
	...
	if (state == UsefulStructures::pip_run_state::predict_taken) {
		if (token.op >= 27 && token.op <=32) MipsSimulator.PC = static_cast<unsigned>(r[2]);
		else MipsSimulator.PC = static_cast<unsigned> (r[1]);
	...
	if (ok) {
		...
		if (r[4] == 0) {
			...
			if (state == UsefulStructures::pip_run_state::predict_taken) {
				...
				MipsSimulator.log << "The prediction get wrong" << endl;
			}
		}
		else if (r[4] == 4 || r[4] == 2) {
			//clog << Get a jump command and clear the instructions before this one" << endl;
