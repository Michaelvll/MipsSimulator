...
void PipelineClass::Data_Preparation(int &state, int busyreg[4])
{
	...
	if (token.op == UsefulStructures::op_num::b || token.op == UsefulStructures::op_num::j || token.op == UsefulStructures::op_num::jal) {
		//clog << "Find a non-conditional jump and change the PC to " << r[0] << endl;
		//MipsSimulator.log << "Find a non-conditional jump and change the PC to " << r[0] << endl;
		MipsSimulator.PC = static_cast<unsigned>(r[0]);
	}
	else if (token.op == UsefulStructures::op_num::jr || token.op == UsefulStructures::op_num::jalr) {
		MipsSimulator.PC = static_cast<unsigned>(r[4]);
	}
