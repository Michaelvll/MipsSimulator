...
ostream &operator<<(ostream &out, const UsefulStructures::Token &token) {
  out << "Token{[op] " << token.op << ", [r]{ " << token.r[0] << ", "
      << token.r[1] << ", " << token.r[2] << " }, [rstate]{ " << token.rstate[0]
      << ", " << token.rstate[1] << ", " << token.rstate[2] << " }, [offset]"
      << token.offset << " }";
  return out;
}
...
ostream &operator<<(ostream &out, const UsefulStructures::arguToken &token) {
  out << "arguToken{ [r]{ " << token.r[0] << ", " << token.r[1] << ", "
      << token.r[2] << " }, [rstate]{ " << token.rstate[0] << ", "
      << token.rstate[1] << ", " << token.rstate[2] << " }, [offset]"
      << token.offset << " }";
  return out;
}
...
UsefulStructures::arguToken &UsefulStructures::arguToken::
operator=(const Token &rt) {
  for (int i = 0; i < 3; ++i) {
    r[i] = rt.r[i];
    rstate[i] = rt.rstate[i];
  }
