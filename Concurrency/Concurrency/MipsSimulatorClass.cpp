...
inline T MipsSimulatorClass::Get_Next_Num(const string &s, size_t &pos) {
	...
	for (; pos < s.length() && (s[pos] == ' ' || s[pos] == ',' || s[pos] == '\t');
		++pos)
		;
...
inline string MipsSimulatorClass::Get_Next_String(const string &s,
	size_t &pos) {
	string op;
	for (; pos < s.length() && (s[pos] == ' ' || s[pos] == ',' || s[pos] == ':' ||
		s[pos] == '\t' || s[pos] == '(');
...
bool MipsSimulatorClass::Text_labelProcess(const string &s, state_num &state) {
	...
	if (pos < s.length() && s[pos] == ':') {
		...
		// clog << "Get a text_label: \"" << op << "\" of the expr_pos  " <<
		// expr_pos << endl;
		log << "Get a text_label: \"" << op << "\" of the expr_pos  " << expr_pos
			<< endl;

		++pos;
		for (; pos < s.length(); ++pos) {
			string op1 = Get_Next_String(s, pos);
			txt_lab_tab[op1] = expr_pos; // Record the position of the label in text
										 // with unordered_map
			// clog << "Get a text_label: \"" << op << "\" of the expr_pos  " <<
			// expr_pos << endl;
			log << "Get a text_label: \"" << op1 << "\" of the expr_pos  " << expr_pos
				<< endl;
		}
		return true;
	}
...
void MipsSimulatorClass::Data_Process(string s, state_num &state) {
	...
	if (pos < s.length() && s[pos] == ':') {
		...
		// clog << "Get a mem_label: \"" << op << "\" of the mem_pos " << mem_pos <<
		// endl;
		log << "Get a mem_label: \"" << op << "\" of the mem_pos " << mem_pos
			<< endl;
		...
		for (; pos < s.length(); ++pos) {
			string op1 = Get_Next_String(s, pos);
			mem_lab_tab[op1] =
				mem_pos; // Record the position of the mem label with unordered_map
			// clog << "Get a mem_label: " << op << " of the mem_pos " << mem_pos <<
			// endl;
			log << "Get a mem_label: " << op1 << " of the mem_pos " << mem_pos
				<< endl;
		}
	}
	else if (op[0] == '.') {
		if (op == ".data") {
			...
			// clog << "Put the string (without end signal) " << str << " into the
			// memory at: " << mem_pos << endl;
			log << "Put the string (without end signal) " << str
				<< " into the memory at: " << mem_pos << endl;
		}
		else if (op == ".asciiz") {
			// string oristr = Get_Next_String(s, pos);
			string str = String_Fetch(s, pos);
			...
			memory[mem_pos++] = '\0';
			// clog << "Put the string (with end signal) " << str << " into the memory
			// at: " << mem_pos << endl;
			log << "Put the string (with end signal) " << str
				<< " into the memory at: " << mem_pos << endl;
		}
		else if (op == ".byte") {
			vector<char> vstr;
			...
			// clog << "into the memory and the mem_pos is changed to " << mem_pos <<
			// endl;
			log << "into the memory and the mem_pos is changed to " << mem_pos
				<< endl;
		}
		else if (op == ".half") {
			vector<short> vn;
...
void MipsSimulatorClass::readcode(std::istream &codein) {
	/// Process the Mips code from the stream codein, which comes from a whole
	/// file
	/// Parse the code into a kind of language that can be easily recognized by
	/// the machine
	...
	// Execute all the command in .data and record the mem_label into a unordered
	// map
	// Put the commands in .text into a vector<string> preExpr and record the
	// text_label into a unordered map
	string tmps;
	...
	while (getline(codein, tmps)) {
		...
		if (state == state_num::text) {
			...
			// clog << "Start a Code_Process of the Line " << linenum << " in " <<
			// (state == state_num::data ? "data" : "text") << " mod" << endl;
			log << "Start a Code_Process of the Line " << linenum << " in "
				<< (state == state_num::data ? "data" : "text") << " mod" << endl;
			...
			// clog << "Get a command: " << tmps << " and push it to the vector of
			// preExpr" << endl;
			log << "Get a command: " << tmps
				<< " and push it to the vector of preExpr" << endl;
		}
		else {
			// state == data
	...
	for (auto x : preExpr) {
		...
		log << "-------------------------------------------------------------------"
			"--------------------------------"
			<< endl;
		// clog << "Start a Token_Process of the Line " << nowline << " in preEpr"
		// << endl;
		log << "Start a Token_Process of the Line " << nowline << " in preEpr"
			<< endl;
		...
		for (int i = 0; i < 3; ++i) {
			...
			if (r[i] == "")
				rstate[i] = UsefulStructures::r_state::none;
			else if (isReg(r[i]))
				rstate[i] = UsefulStructures::r_state::regi;
			else
				rstate[i] = UsefulStructures::r_state::immi;
		}
			...
			if (rstate[i] == UsefulStructures::r_state::immi) {
				...
				if (r[i][0] == '-' || (r[i][0] <= '9' && r[i][0] >= '0')) {
					...
					if (txt_lab_tab.find(r[i]) != txt_lab_tab.end()) {
						n = txt_lab_tab[r[i]];
						// clog << "The r[" << i << "] is a text_label \"" << r[i] << "\"
						// represent the Line " << n << endl;
						log << "The r[" << i << "] is a text_label \"" << r[i]
							<< "\" represent the Line " << n << endl;
					}
					else {
						n = mem_lab_tab[r[i]];
						// clog << "The r[" << i << "] is a mem_label \"" << r[i] << "\"
						// represent the mem_pos" << n << endl;
						log << "The r[" << i << "] is a mem_label \"" << r[i]
							<< "\" represent the mem_pos" << n << endl;
					}
					token.r[i] = n;
				}
				...
				int n;
				size_t t = 0;
				if (reg_name[0] <= '9' && reg_name[0] >= '0')
					n = Get_Next_Num<int>(reg_name, t);
				else
					n = reg_num_tab[reg_name];

				token.r[i] = n;
				token.rstate[i] = UsefulStructures::r_state::regi;

				// clog << "The r[" << i << "] is a register " << reg_name << "
				// represent the reg_num " << n;
				log << "The r[" << i << "] is a register " << reg_name
					<< " represent the reg_num " << n;
...
void MipsSimulatorClass::pipeline() {
	...
	// std::thread IF(stage.Instruction_Fetch());
	PC = txt_lab_tab["main"];
	...
	mutex rdy;
	bool ready[5] = { 1,1,1,1,1 };
	condition_variable rdycv;

	thread IF([&] {
		while (true) {
			unique_lock<mutex> lk(rdy);
			rdycv.wait(lk, [&] {return !ready[0]; });
			lk.unlock();
	...
	while (true) {
		...
		if (!usefulstructures.Busy(UsefulStructures::pc, MipsSimulator.busyreg) &&
			PC < expr.size() && !cache.IFdat.valid) {
			cache.IFdat.PC = PC;
			cache.IFdat.valid = true;
		}
		...
		while (true) {
			lk.lock();
			if (ready[0] && ready[1] && ready[2] && ready[3] && ready[4]){
				lk.unlock();
				break;
			}
			lk.unlock();
		...
		else if (run_state == UsefulStructures::pip_run_state::pause) {
			cache.EXdat.valid = false;
			cache.MAdat.Change(tmpcache.MAdat);
			cache.WBdat.Change(tmpcache.WBdat);
		}
		else if (run_state == UsefulStructures::pip_run_state::jump) {
			cache.IFdat.valid = false;
			cache.IDdat.valid = false;
			cache.EXdat.Change(tmpcache.EXdat);
			cache.MAdat.Change(tmpcache.MAdat);
			cache.WBdat.Change(tmpcache.WBdat);
		}
		else if (run_state == UsefulStructures::pip_run_state::wait) {
			cycle += 5;
		}
		else {
			lk.lock();
